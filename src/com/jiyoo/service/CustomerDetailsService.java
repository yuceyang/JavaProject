...
public class CustomerDetailsService extends HttpServlet{
	private static final long serialVersionUID = 1L;
	@SuppressWarnings("static-access")
	@Override
	protected void service(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		if(req.getParameter("bianhao")==""||req.getParameter("bianhao")==null){
			resp.sendRedirect("jsp/the.jsp");//重定向页面
		}else{
			StringUtf u=new StringUtf();
			//1.0
			BigPackage bp=new BigPackage();
			bp.setDaquusername(u.U(req.getParameter("daquusername")));
			bp.setMdaqu(u.U(req.getParameter("mdaqu")));
			...
			new FactoryList().customerDetailsDao().cust(bp);
			resp.sendRedirect("jsp/kehuxinxi.jsp");//重定向页面
		}
	}
}
